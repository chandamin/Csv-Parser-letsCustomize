  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        ...
        if (data?.data?.csvUrl) {
          setSavedCsvUrls([data.data.csvUrl]);
        }
      } catch (err) {
  ...
  const onUpdatePrices = useCallback(async () => {
    try {
      ...
      const priceHeader =
        headers.find(
          (h) =>
            String(h).trim().toLowerCase() === "recommended price"
        ) || "recommended price";
        ...
        <BlockStack gap="400">
          ...
          <Card>
            <BlockStack gap="400">
              ...
              {savedCsvUrls.length > 0 && (
                <Card>
                  <BlockStack gap="200">
                    ...
                    {savedCsvUrls.map((url, index) => (
                      <Button
                        key={index}
                        onClick={() => setCsvUrl(url)}
                      >
                        Use saved CSV
                      </Button>
                    ))}
              ...
              <Button variant="primary" onClick={loadCsvFromUrl}>
                Load CSV
              </Button>
            </BlockStack>
          </Card>
          ...
          {rows.length > 0 && (
            <Card>
              <BlockStack gap="300">
                ...
                {showPreview && (
                  <Button variant="primary" onClick={onUpdatePrices}>
